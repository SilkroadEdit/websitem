...
if ( ($ItemSID != 0) && ($ItemSID != NULL) ){
		...
		//SwitchColors
		if($grade == "Seal of Star" || $grade == "Seal of Moon" || $grade == "Seal of Sun" || $grade == "Fight" || $grade == "Power") {
		...
		if($grade == "Seal of Star" || $grade == "Seal of Moon" || $grade == "Seal of Sun") {
		...
		**************************************************/
		...
		if($fetchGetItem['MagParamNum'] > 0) {
			...
			for($x = 1; $x <= $fetchGetItem['MagParamNum']; $x++) 
			{
				...
				$BluesQry = $users->link->db_conn_shard->query("SELECT * FROM SRO_VT_SHARD.._RefMagicOpt where ID = '$MagParams[Mag1ID]';");
				$BlueParam = $BluesQry->fetch(PDO::FETCH_ASSOC);
